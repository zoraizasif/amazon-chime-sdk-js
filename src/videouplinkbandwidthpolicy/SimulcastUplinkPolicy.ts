...
export default class SimulcastUplinkPolicy implements VideoUplinkBandwidthPolicy {
  ...
  constructor(
    ...
    private disableSimulcastP2P: boolean,
  ...
  private calculateEncodingParameters(
    ...
    if (this.currentActiveStreams === ActiveStreams.kHi) {
      // Don't trigger redetermination based on rate if simulcast disabled
    ...
    if (
      ...
      if (this.disableSimulcastP2P && this.numParticipants >= 0 && this.numParticipants <= 2) {
        // Simulcast disabled
  ...
  updateIndex(videoIndex: VideoStreamIndex): void {
    ...
    let numParticipantsChanged = false;
    if (this.disableSimulcastP2P) {
